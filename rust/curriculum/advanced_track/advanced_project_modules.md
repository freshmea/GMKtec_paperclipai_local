...
- M1을 진행할 때는 `ripgrep`를 기준 사례로 읽고, `fd`를 UX 비교 사례로 함께 읽는다.
- M2를 진행할 때는 `Tokio`를 중심으로 보고, `Deno`를 대규모 runtime product 사례로 비교한다.
- M3를 진행할 때는 `uv`를 기준으로 보고, `Serde`, `Tauri`를 추상화와 product integration 사례로 확장한다.
- M4를 진행할 때는 `rustls`와 `Tokio`를 함께 읽어 reliability와 safety 관점을 보강한다.
